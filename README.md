...
* **A real trained model runs entirely on the NPU**: `examples/mel-kws/` trains a
  ...
  **98.00%** INT8 test accuracy, 191,968 / 192,000 board-exact output bytes, 2.54 ms per
  utterance.
* **1,167 host tests at 99.18% compiler line coverage**, a **2,341-model container baseline** (every published suite model
  still compiles to identical bytes), and a **130-row board ledger** counting
...
PYTHONPATH=src python research/verify_suites.py   # 2,341 models vs the checked-in baseline
...
| `tests/` | 1,167 host tests (99.18% line coverage, floor 99% in CI) plus the C board harnesses (`board_*.c`) |
