"""MIT. Depthwise-layer-in-chain regression (depthwise-separable blocks)."""
...
SUITE = Path(__file__).resolve().parents[1] / "research" / "depthwise_chain_suite"
D, W = "dense", "depthwise"


def build(branches, expression, hidden=3):
    """A stem, branch layer chains and joins over produced tensor names."""
...
class DepthwiseChainTests(unittest.TestCase):
    def test_recompiles_byte_identically(self):
        ...
        for entry in manifest:
            ...
            with self.subTest(model=entry["index"]):
                ...
                self.assertEqual([len(branch) for branch in meta["branch_names"]],
                                 [len(branch) for branch in entry["branches"]])
    ...
    def test_chained_depthwise_is_expanded_to_dense(self):
        _, meta = compile_sequence(build([[(D, 3, 8, 1), (W, 8, 8, 3), (D, 8, 3, 1)],
                                          [(D, 3, 3, 1)], [(D, 3, 3, 1)]],
                                         [("Add", "b0_2", "b1_0"), ("Mul", "j0", "b2_0")]))
        self.assertEqual([len(branch) for branch in meta["branch_names"]], [3, 1, 1])
        self.assertTrue(all(branch["weight_zero_points"] is not None
                            for branch in meta["branch_quantization"][0]))

    def test_rejections(self):
        # A join may only consume three-channel tensors.
        with self.assertRaises(ValueError):
            compile_sequence(build([[(D, 3, 8, 1)], [(D, 3, 8, 1)], [(D, 3, 3, 1)]],
                                   [("Add", "b0_0", "b1_0"), ("Mul", "j0", "b2_0")]))
        # A chained depthwise layer with a 5x5 kernel is outside the dense expansion.
        with self.assertRaises(ValueError):
            compile_sequence(build([[(D, 3, 8, 1), (W, 8, 8, 5), (D, 8, 3, 1)],
                                    [(D, 3, 3, 1)], [(D, 3, 3, 1)]],
                                   [("Add", "b0_2", "b1_0"), ("Mul", "j0", "b2_0")]))
        # A single-layer depthwise branch still needs a three-channel stem.
        with self.assertRaises(ValueError):
            compile_sequence(build([[(W, 3, 3, 3)], [(D, 3, 3, 1)], [(D, 3, 3, 1)]],
                                   [("Add", "b0_0", "b1_0"), ("Mul", "j0", "b2_0")], hidden=8))
        with self.assertRaises(ValueError):
            compile_sequence(build([[(D, 3, 8, 1), (W, 8, 8, 3), (D, 8, 3, 1)],
                                    [(D, 3, 3, 1)], [(D, 3, 3, 1)]],
                                   [("Add", "b0_2", "b1_0"), ("Mul", "j0", "b2_0")]),
                             calibration_ranges={})
