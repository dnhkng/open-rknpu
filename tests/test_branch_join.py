"""MIT. Multi-layer branch join regression (residual-style branch chains)."""
...
SUITE = Path(__file__).resolve().parents[1] / "research" / "branch_join_suite"
D, W = "dense", "depthwise"
...
class BranchJoinTests(unittest.TestCase):
    ...
    def test_rejections(self):
        ...
        # A branch whose final layer keeps eight channels cannot be joined.
        with self.assertRaises(ValueError):
            compile_sequence(build([[(D, 3, 8, 1)], [(D, 3, 3, 1)], [(D, 3, 3, 1)]],
                                   [("Add", "b1_0", "b2_0"), ("Mul", "j0", "b1_0")]))
        # Four layers per branch are outside the bounded chain length.
        with self.assertRaises(ValueError):
            compile_sequence(build([[(D, 3, 3, 1)] * 4, [(D, 3, 3, 1)], [(D, 3, 3, 1)]],
                                   [("Add", "b0_3", "b1_0"), ("Mul", "j0", "b2_0")]))
        # A depthwise branch needs a three-channel stem.
        with self.assertRaises(ValueError):
            compile_sequence(build([[(W, 3, 3, 3)], [(D, 3, 3, 1)], [(D, 3, 3, 1)]],
                                   [("Add", "b0_0", "b1_0"), ("Mul", "j0", "b2_0")], hidden=8))
        with self.assertRaises(ValueError):
            compile_sequence(build([[(W, 3, 3, 7)], [(D, 3, 3, 1)], [(D, 3, 3, 1)]],
                                   [("Add", "b0_0", "b1_0"), ("Mul", "j0", "b2_0")]))
        with self.assertRaises(ValueError):
