...
root=Path(__file__).resolve().parent/'conv_asymmetric_suite';root.mkdir(exist_ok=True)
...
for i,(k,sy,sx,padded,ic) in enumerate((k,sy,sx,p,ic) for k in (1,3,5) for sy,sx in ((1,1),(1,2),(2,1),(2,2)) for p in (False,True) for ic in (1,3)):
    ih=5+i%4;iw=5+(i//4)%4;oc=(2,3,4,5,8,16)[i%6];pad=k//2 if padded else 0;pads=[0,k-1,k//2,0] if i%2 else [k-1,0,0,k//2];relu=bool(i%2)
    ...
    margin=k//2
    y=np.stack([reference(np.pad(v,((pads[0],pads[2]),(pads[1],pads[3]),(0,0)),constant_values=zp),q)[margin:ih+pads[0]+pads[2]-margin:sy,margin:iw+pads[1]+pads[3]-margin:sx] for v in x])
