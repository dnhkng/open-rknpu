"""MIT. Verify bounded depthwise K5 ConvTranspose."""
...
base=Path(__file__).resolve().parent;root=base/'transpose_k5_suite';root.mkdir(exist_ok=True);rng=np.random.default_rng(110350);manifest=[]
cases=[([2,2],[2,2,2,2],[0,0]),([1,1],[2,2,2,2],[0,0]),([1,2],[4,0,1,3],[0,1])]
for i,(strides,pads,opad) in enumerate(cases):
    model=onnx.load(base/'depthwise_suite'/f'model{i:03}.onnx');node=model.graph.node[-1];node.op_type='ConvTranspose';del node.attribute[:];node.attribute.extend([h.make_attribute('group',3),h.make_attribute('kernel_shape',[5,5]),h.make_attribute('strides',strides),h.make_attribute('pads',pads),h.make_attribute('output_padding',opad)])
    w=rng.uniform(-.3,.3,(3,1,5,5)).astype(np.float32);b=rng.uniform(-.5,.5,3).astype(np.float32)
    ...
    oh=7*strides[0]+5-pads[0]-pads[2]+opad[0];ow=7*strides[1]+5-pads[1]-pads[3]+opad[1]
    ...
    x.tofile(root/f'input{i:03}.u8');np.stack(ys).tofile(root/f'expected{i:03}.i8');manifest.append(dict(index=i,kernel=5,strides=strides,pads=pads,output_padding=opad,output_shape=[oh,ow,3]))
