...
root=Path(__file__).resolve().parent;out=root/'fixtures/depthwise_h5w6';out.mkdir(exist_ok=True)
shutil.copyfile(root/'depthwise_geometry_suite/model002.onnx',out/'model.onnx')
paths=[]
for i in range(8):
 p=out/f'calibration{i}.npy';np.save(p,np.random.default_rng(i).uniform(0,255,(1,3,5,6)).astype(np.float32));paths.append(str(p))
