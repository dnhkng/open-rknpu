...
root=Path(__file__).resolve().parent/'depthwise_small_channels_suite';root.mkdir(exist_ok=True)
rng=np.random.default_rng(110317)
cases=[(5+(c%4),5+((c+k)%4),k,s,c) for c in range(1,3) for k in (1,3,5) for s in (1,2)]
