"""Two-stage elementwise DAG suite: Mul(Add(a,b), a) with fan-out.

Two independent 1x1 Conv branches feed Add; the Add result and the first branch
are multiplied (fan-out). Independently generated commands; expected bytes come
from the composed integer references. Board runner: tests/board_api.c.
"""
...
parser.add_argument("--directory", default="research/elementwise_chain_suite")
...
for index in range(8):
    ...
    manifest.append({"index": index, "cases": args.cases, "output_scale": meta["output_scale"],
                     "chain": meta["elementwise_chain"], "output_bytes": int(outputs[0].size)})
