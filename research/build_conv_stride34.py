...
root=Path(__file__).resolve().parent/'conv_stride34_suite';root.mkdir(exist_ok=True)
rng=np.random.default_rng(110321);manifest=[]
for i,(k,sy,sx,padded,ic) in enumerate((k,sy,sx,p,ic) for k in (1,3,5) for sy,sx in ((3,3),(4,4),(1,4),(3,2)) for p in (False,True) for ic in (1,3)):
