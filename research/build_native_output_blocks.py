...
root=Path(__file__).resolve().parent/'native_output_blocks_suite';root.mkdir(exist_ok=True)
rng=np.random.default_rng(110321);manifest=[]
for i,(k,oc) in enumerate((k,oc) for k in (1,3,5,7) for oc in (17,31,32,33,48,64)):
    ic=(1,2,3,5,8,16)[i%6]
    sy=sx=1;padded=True
