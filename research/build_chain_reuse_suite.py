"""N-layer chain suite with reused intermediate arena buffers (ping-pong lifetimes).
...
parser.add_argument("--directory", default="research/chain_reuse_suite")
parser.add_argument("--cases", type=int, default=16)
args = parser.parse_args()
root = Path(__file__).resolve().parents[1] / args.directory
root.mkdir(parents=True, exist_ok=True)
rng = np.random.default_rng(110390)
...
for index, layers in enumerate(CONFIGS):
    ...
    binary, meta = compile_sequence(path, reuse_intermediates=True)
...
print(f"Built {len(manifest)} chain-reuse models in {root}")
