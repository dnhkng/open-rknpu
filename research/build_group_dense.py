...
root=Path(__file__).resolve().parent/'group_dense_suite';root.mkdir(exist_ok=True)
rng=np.random.default_rng(110321);manifest=[]
cases=[(g,m,k,d) for g in (2,3,4,5,7,8) for m in (1,2) if g*m<=16 for k,d in ((1,(1,1)),(2,(1,2)),(3,(2,1)),(3,(2,2)))]
for i,(group,mult,k,dilation) in enumerate(cases):
    ic=2*group;sy,sx=((1,1),(1,2),(2,1),(2,2))[i%4];padded=True
    ...
    w=rng.uniform(-.3,.3,(oc,2,k,k)).astype(np.float32);b=rng.uniform(-1,1,oc).astype(np.float32)
