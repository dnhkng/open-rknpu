"""Native input C65..128 suite (five to eight 16-lane planes).
...
The weight layout for these plane counts is the 32-lane-part generalization of
the C33..64 rule, recovered from the vendor marker captures
`capture_native_c65_channels`, `capture_native_c65_oci` and
`capture_native_c128_channels` and reproduced by `tests/test_native_c48.py`.
"""
...
parser.add_argument("--directory", default="research/native_c65_suite")
parser.add_argument("--cases", type=int, default=16)
args = parser.parse_args()
root = Path(__file__).resolve().parents[1] / args.directory
root.mkdir(parents=True, exist_ok=True)
rng = np.random.default_rng(110650)
...
for ic in (65, 80, 96, 128):
    for oc, kernel, height, width in ((1, 1, 6, 5), (8, 3, 6, 5), (16, 3, 8, 8), (3, 1, 8, 8),
        ...
        graph = h.make_graph([node], "native_c65",
...
print(f"Built {len(manifest)} native C65..128 models in {root}")
