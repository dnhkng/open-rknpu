...
root=Path(__file__).resolve().parent/'leaky_verified_suite';root.mkdir(exist_ok=True);rng=np.random.default_rng(110324);manifest=[]
for i in range(24):
    k=1;height=5+i%4;width=5+(i//4)%4;ic=1 if i%2 else 3;oc=(1,3,5,16)[i%4];alpha=(0.,.1,.2,.5,1.,.01)[i%6]
    ...
    try:data,meta=compile_sequence(p,scale,zp)
    except ValueError as e:
        print(i,str(e));continue
