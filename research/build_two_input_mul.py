...
root=Path(__file__).resolve().parent/'two_input_mul_suite';root.mkdir(exist_ok=True);rng=np.random.default_rng(110326);manifest=[]
for i in range(12):
    height=5+i%4;width=5+(i//4);channels=(3,5,8,16)[i%4]
    nodes=[];constants=[]
    for branch in ('a','b'):
        w=rng.uniform(-.3,.3,(channels,3,1,1)).astype(np.float32);b=rng.uniform(-2,2,channels).astype(np.float32)
        constants.extend([nh.from_array(w,branch+'w'),nh.from_array(b,branch+'b')]);nodes.append(h.make_node('Conv',[branch,branch+'w',branch+'b'],[branch+'out'],kernel_shape=[1,1]))
    nodes.append(h.make_node('Mul',['aout','bout'],['output']))
