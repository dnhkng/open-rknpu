"""MIT. Verify Conv->Relu branches -> Mul -> Relu integer ordering."""
...
root=Path(__file__).resolve().parent/'mul_relu_order_suite';root.mkdir(exist_ok=True);rng=np.random.default_rng(110377);manifest=[]
for i,(c,modes) in enumerate(((3,(1,0)),(5,(0,1)),(9,(1,1)),(16,(1,1)))):
    ...
    nodes.extend([h.make_node('Mul',outs,['product']),h.make_node('Relu',['product'],['output'])])
    model=h.make_model(h.make_graph(nodes,'mul_relu_order',[h.make_tensor_value_info('input',1,[1,3,height,width])],[h.make_tensor_value_info('output',1,[1,c,height,width])],initial),opset_imports=[h.make_opsetid('',13)]);model.ir_version=8
    ...
    for v in x:
        y=mul_requant_reference(reference(v,qs[0]),reference(v,qs[1]),conversion,shift,requested['zero_point'])
        ys.append(np.maximum(y,requested['zero_point']).astype(np.int8))
    np.stack(ys).tofile(root/f'expected{i:03}.i8');manifest.append(dict(index=i,input_scale=1.,input_zero_point=0,compile_output_range=requested,branch_relu=list(modes),**meta))
