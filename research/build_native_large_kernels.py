...
root=Path(__file__).resolve().parent/'native_large_kernels_suite';root.mkdir(exist_ok=True)
rng=np.random.default_rng(110321);manifest=[]
for i,(k,ic) in enumerate((k,ic) for k in (9,11,13,15) for ic in (1,3,16)):
    oc=(1,3,8)[i%3]
    sy=sx=1;padded=True
