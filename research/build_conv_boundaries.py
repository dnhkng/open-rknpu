...
root=Path(__file__).resolve().parent/'conv_boundary_suite';root.mkdir(exist_ok=True)
...
for i,(k,sy,sx,padded,ic) in enumerate((k,sy,sx,p,ic) for k in (1,3,5) for sy,sx in ((1,1),(1,2),(2,1),(2,2)) for p in (False,True) for ic in (1,3)):
    ih=5+i%4;iw=5+(i//4)%4;oc=1 if i%2==0 else 3;pad=k//2 if padded else 0;relu=bool(i%2)
