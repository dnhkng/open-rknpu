"""MIT. Verify bounded depthwise K2/K3 unequal-stride ConvTranspose."""
...
base=Path(__file__).resolve().parent;root=base/'transpose_unequal_suite';root.mkdir(exist_ok=True);rng=np.random.default_rng(110347);manifest=[]
cases=[(2,[1,2],{'pads':[0,0,0,0],'output_padding':[0,1]},[0,0,0,0]),
       (2,[2,1],{'pads':[1,0,0,1],'output_padding':[1,0]},[1,0,0,1]),
       (3,[1,2],{'pads':[1,0,1,1]},[1,0,1,1]),
       (3,[2,1],{'pads':[0,1,1,1]},[0,1,1,1]),
       (2,[1,2],{'auto_pad':'SAME_UPPER','output_padding':[0,1]},[0,0,1,1]),
       (2,[2,1],{'auto_pad':'SAME_LOWER','output_padding':[1,0]},[1,1,0,0])]
for i,(k,strides,extra,pads) in enumerate(cases):
    model=onnx.load(base/'depthwise_suite'/f'model{i:03}.onnx');node=model.graph.node[-1];node.op_type='ConvTranspose';del node.attribute[:]
    attrs=dict(group=3,kernel_shape=[k,k],strides=strides,**extra);node.attribute.extend(h.make_attribute(name,value) for name,value in attrs.items())
    ...
    opad=extra.get('output_padding',[0,0]);oh=7*strides[0]+k-pads[0]-pads[2]+opad[0];ow=7*strides[1]+k-pads[1]-pads[3]+opad[1]
    ...
    x.tofile(root/f'input{i:03}.u8');np.stack(ys).tofile(root/f'expected{i:03}.i8');manifest.append(dict(index=i,kernel=k,strides=strides,attributes=extra,resolved_pads=pads,output_shape=[oh,ow,3]))
