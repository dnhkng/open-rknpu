...
root=Path(__file__).resolve().parent/'native_input_suite';root.mkdir(exist_ok=True)
rng=np.random.default_rng(110321);manifest=[]
for i,(k,ic) in enumerate((k,ic) for k in (1,3,5) for ic in (2,4,5,8,9,16)):
    sy=sx=1;padded=True
    ih=5+i%4;iw=5+(i//4)%4;oc=(2,3,4,5,8,16)[i%6];pad=k//2 if padded else 0;relu=False
