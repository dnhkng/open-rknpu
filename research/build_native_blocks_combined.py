...
root=Path(__file__).resolve().parent/'native_blocks_combined_suite';root.mkdir(exist_ok=True)
...
for i,(k,ic) in enumerate((k,ic) for k in (1,3,5,7) for ic in (17,18,23,24,31,32)):
    ...
    ih=(5,9,16,32)[i%4];iw=(7,9,17,32)[i%4];pad=k//2 if padded else 0;relu=bool(i%2)
    ...
    x=rng.integers(0,256,(3,ih,iw,ic),dtype=np.uint8);x[0]=0;x[1]=255;x[2]=rng.integers(0,256,(ih,iw,ic),dtype=np.uint8)
    margin=k//2-pad
