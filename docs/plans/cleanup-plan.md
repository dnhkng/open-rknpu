...
873 host tests, the 121-row board ledger, the campaign sweep baseline (157 same / 12 diff
...
1. all 2,244 `research/*suite*/model*.onnx` recompile to the checked-in baseline
   ...
   `ERR:ValueError`;
2. `python -m unittest discover -s tests` and `python -m pytest tests -q`: **873 passing**
