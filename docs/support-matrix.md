...
| `MatMul`/`Gemm` (dense layer) | **supported** when the weight is a constant rank-2 tensor and the activation is `[N,C,1,1]` or flattened from it: lowered to the verified 1×1 Conv path; `Softmax`, `ReduceMean`, `Concat`, `Slice`, `Pow`, `Sqrt` and `Shape`-driven control flow still have no primitive (`sequence lowering requires one input, one output, and an initial Conv`) | `../research/matmul_suite/` (12 models: direct, Flatten, Reshape, Gemm ± bias, transB=1) | board: 12 models / 192 inferences / 1,152 exact bytes; host byte-equality with the hand-written 1×1 Conv |
