...
| one constant-parameter `QLinearConv` | UINT8 input, INT8 output, scalar activation parameters, scalar or per-output-channel weight scale/zero point, optional INT32 bias; source INT8 weight bytes preserved bit-for-bit; geometry is the native bounds (batch 1..16, H/W 1..128, C1..16352, odd K1..31, stride 1..4, dilation 1..17) plus preserved-quantization range checks; board-verified on C1..32→C1..64 and K1/3/5 sample points | `quantized_import.py` | — | `board: qlinearconv_import_suite` |
...
| Conv input channels > 16352 | `native Conv requires static batch1..16, H/W1..128, input C1..16352, constant weights/bias` | **measured wall**: the weight table has at most 511 32-lane parts, so C16352 (511 parts) is exact and C16368 (512 parts) never completes (job timeout + driver soft reset). Wide channels shrink the per-task image (6144 input atoms), so height tiling takes over. See [investigation-log.md](investigation-log.md) | `../research/wide_channel_suite/` (C129..16352, 13 models / 45 inferences / 168,994 exact bytes) |
| Conv output channels > 8192 | `native Conv supports odd K1..31, explicit padding, stride 1..4, input C1..16352/output C1..8192` | **measured wall**: the surface block index `(oc-1)//16` is nine bits. C8192 (512 blocks) is exact; C16384 writes the first 8192 channels correctly and then wrong bytes | `../research/wide_channel_suite/` (`c8192-out-k1`) |
| Kernels > 31 | `native Conv supports odd K1..31, explicit padding, stride 1..4, input C1..16352/output C1..8192` | [roadmap.md](roadmap.md) (kernels > 31; the K33 mismatch) | `../research/native_kernel_limits_suite/` |
