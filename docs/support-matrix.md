...
| three-stage reduction to 1×1 | the legacy `network` profile (`Conv,Relu,Conv` + three matching 2×2 stride-2 pools → float32 `[1,3,1,1]`) and the `reduction` profile (a Conv followed by three matching pools) | legacy profile 5/6/7/8 | `../examples/primitives/02_conv_pool.py` | `host:` legacy path |
...
| --- | --- | --- | --- |
| 1-D convolution (`kernel_shape [k]`, rank-3 input) | the front end requires static rank-4 NCHW: `static NCHW input required` | [roadmap.md](roadmap.md) "Deliberately out of scope" (1-D convolution); [plans/primitive-roadmap.md](plans/primitive-roadmap.md) Silero VAD | [plans/primitive-roadmap.md](plans/primitive-roadmap.md) (the measured Silero VAD envelope) |
...
| `MatMul`, `Gemm`, `Softmax`, `ReduceMean`, `Concat`, `Slice`, `Pow`, `Sqrt`, `Shape`-driven control flow | no primitive; `sequence lowering requires one input, one output, and an initial Conv` or a profile-specific error | [roadmap.md](roadmap.md) "Deliberately out of scope" | `../examples/mel-kws/README.md` (host-side head pattern) |
